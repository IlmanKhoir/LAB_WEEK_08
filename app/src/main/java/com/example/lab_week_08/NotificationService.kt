...
class NotificationService : Service() {
    // In order to make the required notification, a foreground service is required
    // to show the ongoing notification while the work runs.
    ...
    override fun onCreate() {
        ...
        // Create a HandlerThread and associated Handler for background work
        val handlerThread = HandlerThread("SecondThread").apply { start() }
        serviceHandler = Handler(handlerThread.looper)
    }

    // Custom function to build and start the foreground notification
    ...
    // This callback will be called when the service is started (after startForeground)
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        // Post the notification task to the handler so it runs off the main thread
        serviceHandler.post {
            // Count down from 10 to 0 in the notification
            countDownFromTenToZero(notificationBuilder)

            // Notify the MainActivity that the service process is done
            notifyCompletion(Id)
